...
export class IncidentService {
    ...
    async createIncident(newIncident: Incident): Promise<Incident> {
        await this.repo
            ...
            .then((exists) => {
                if (exists)
                    throw new HttpException(
                        `Incident with ID ${newIncident.incidentId} already exists!`,
    ...
    async updateIncident(routeId: number, incidentToUpdate: Incident) {
        ...
        if (routeId != incidentToUpdate.incidentId) {
            throw new HttpException(
        ...
        await this.repo
            .exists({
                where: {
                    incidentId: incidentToUpdate.incidentId,
                },
            })
            .then((exists) => {
                if (!exists)
                    throw new HttpException(
                        `Incident with ID ${incidentToUpdate.incidentId} does not exist!`,
                        HttpStatus.NOT_FOUND,
                    )
            })
